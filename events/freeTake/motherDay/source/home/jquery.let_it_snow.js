!function($){
  ...
  $.fn.let_it_snow = function(options){
    ...
    function snow() {
      ...
      for (var i = 0; i < flakeCount; i++) {
        ...
        if (settings.image == false) {
          var length=$(".list_flake").length;
          var idx=Math.floor(Math.random()*length);
          if(flake.index>-1){
            idx=flake.index;
          }
          flake.index=idx;
          ctx.drawImage($(".list_flake").get(idx), flake.x, flake.y, flake.size * 2, flake.size * 2);
   ...
   function reset(flake) {
    if(settings.total&& parseInt(settings.total-settings.count)>=0){
      if(teptotal>=0){
        ResetFlake(flake);
        teptotal--;
      }else{
  ...
  var teptotal=0;var tempcurrentnum=settings.count;
  if(settings.total&& parseInt(settings.total-settings.count)>=0){
   teptotal=settings.total-settings.count;
 }
