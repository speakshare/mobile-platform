...
module.exports= (function() {
	...
	function MobileSelect(config) {
		...
		this.jsonData = [];
		this.checkDataType();
		this.renderWheels(this.wheelsData,config);
		this.displayJson = []; 
		this.cascade = false;
		this.startY;
		this.moveEndY;
	...
	MobileSelect.prototype = {
		...
		init: function(config){
		    ...
		    _this.trigger.addEventListener('touchend',function(){
		...
		renderWheels: function(wheelsData,config){
			var _this = this;
			...
			_this.mobileSelect.innerHTML = 
		        ...
		        '<div class="content">'+
		            '<div class="btnBar">'+
		                '<div class="fixWidth">'+
		                    '<div class="cancel">'+(config.cancelTxt||"取消")+'</div>'+
		                    '<div class="title"></div>'+
		                    '<div class="ensure">'+(config.comfirmTxt||"选择")+'</div>'+
		                '</div>'+
		            '</div>'+
		            '<div class="panel">'+
		                '<div class="fixWidth">'+
		        ...
		        '</div>';
			if(config.el){
				var elObj=document.getElementById(config.el);
				if(elObj==null){
                    elObj=document.createElement("div");
                    elObj.id=config.el;
				}
                elObj.appendChild(_this.mobileSelect);
		...
		addListenerWheel: function(theWheel, index){
			...
			// theWheel.addEventListener('touchmove', throttle(function (event) {
			// 	_this.touch(event, this.firstChild, index);
			// },50),false);
